# -*- coding: utf-8 -*-
"""生成 cas.db：约 210 名虚拟师生（含管理员 userId=1、插班生 颜文轩）。

字段全部为程序生成的假数据；FLAG3 藏于 颜文轩.description。
同时把"拦截到的登录密文"写入 data/captured_login.txt（题 2 素材，doc.html
调试记录直接展示该密文，不另发附件）。
"""
import json
import os
...
# 独立部署版（leakedkey）：本题只需 FLAG2（密文明文）；题3 的 flag 已替换为
# 无害占位符——颜文轩档案兑换码显示"已回收"。
FLAG3 = os.environ.get("FLAG3", "已回收")
FLAG2 = os.environ.get("FLAG") or os.environ.get("FLAG2", "hctf{fr0nt3nd_rsa_pr1v4te_k3y}")
...
def make_row(rng, uid, name, org, grade, is_admin=False, is_transfer=False):
    ...
    row = {
        ...
        "status": 1,
        "nation": rng.choice(["汉族", "汉族", "汉族", "瑶族", "土家族"]),
        "political": rng.choice(["共青团员", "共青团员", "中共党员", "群众"]),
        "idCard": f"43{rng.randint(10, 99)}0{rng.randint(1970, 2007):04d}{rng.randint(1, 12):02d}{rng.randint(1, 28):02d}{rng.randint(1000, 9999)}{rng.choice(string.digits)}X",
        "birthday": f"{rng.randint(1970, 2007)}-{rng.randint(1, 12):02d}-{rng.randint(1, 28):02d}",
        "nativePlace": rng.choice(["湖南岳阳", "湖南长沙", "湖南株洲", "广东深圳", "江西南昌", "广西桂林"]),
        ...
        "address": rng.choice(["湖南省长沙市麓山南路", "湖南省株洲市天元区", "湖南省长沙市岳麓区"]),
...
def main():
    ...
    # 目标：插班生 颜文轩（userId 固定为 20258888，便于核对）
    rows.append(make_row(rng, 20258888, "颜文轩", ORGS[0], "2023", is_transfer=True))
